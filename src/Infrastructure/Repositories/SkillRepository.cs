...
namespace CDN.FreelancerAPI.Infrastructure.Repositories
{
    public class SkillRepository : ISkillRepository
    {
        ...
        public async Task<IEnumerable<Skillset>> GetAllAsync()
        {
            ...
            const string sql = "SELECT Id, SkillName FROM SkillSetOptions ORDER BY SkillName";
        ...
        {
            using var conn = new SqlConnection(_connectionString);
            const string sql = "INSERT INTO SkillSetOptions (SkillName) VALUES (@SkillName)";
            await conn.ExecuteAsync(sql, new { SkillName = skill.SkillName.Trim() });
        }
